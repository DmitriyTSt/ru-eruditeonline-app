...
fun HtmlText(
    ...
    val htmlInfo = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
        Html.fromHtml(text, Html.FROM_HTML_MODE_LEGACY)
    } else {
        @Suppress("DEPRECATION")
        Html.fromHtml(text)
    }
    val annotatedText = htmlInfo.toAnnotatedString(MaterialTheme.colorScheme.primary)
    ...
    URL {
        ...
        override fun copySpan(
            ...
            destination.addStyle(
                style = SpanStyle(color = context.primaryColor),
                start = start,
                end = end,
            )
        }
